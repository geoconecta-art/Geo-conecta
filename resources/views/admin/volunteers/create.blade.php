        ...
        <div class="components-preview wide-md mx-auto">
            ...
            <div class="nk-block-head">
                <div class="nk-block-head-content">
                    <h4 class="nk-block-title">Crear Incorporación Voluntaria</h4>
                </div>
            </div>

            @if ( Auth::user()->hasRole('Super Administrador') || ( Auth::user()->hasRole('Administrador') &&  !Auth::user()->hasRole('Consultor') ) )
            <form id="personForm" action="{{ route('volunteers.store-by-admin') }}" method="POST" enctype="multipart/form-data" >
            @else
            <form id="personForm" action="{{ route('volunteers.store') }}" method="POST" enctype="multipart/form-data" >
            @endif
                                                ...
                                                <div class="form-control-wrap">
                                                    <div class="form-control-select">
                                                        <select class="form-control" 
                                                            ...
                                                            <option value="1">SERVIDOR PROMOTOR</option>
                                                            <option value="2">LIDERAZGO SECCIONAL</option>
                                                            <option value="3">LIDERAZGO MILITANTE</option>
                                                        </select>
                                                    </div>
                                                </div>
                                            </div>
...
@section('script')
    ...
    @if ( !$errors->any() )
        ...
        var storeIns = localStorage.getItem('storeVol');

        if ( storeIns == 1 ) {

            localStorage.setItem('storeVol', 0);
