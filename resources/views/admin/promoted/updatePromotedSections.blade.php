                                                    ...
                                                    <select class="form-select" name="sections[]" multiple required>
                                                        @foreach ($sections as $section)
                                                            <option value="{{ $section->section }}"
                                                                @if (isset($p_sections) && in_array($section->section, $p_sections)) selected @endif>
                            ...
                            <div class="row mb-5 px-4 mt-3 mt-lg-0">
                                ...
                                <div class="col-12 text-right">
                                    ...
                                    @if (!Auth::user()->hasRole('Consultor'))
                                        <div onclick="updateSections()" class="btn btn-outline-light mr-md-2 mb-3 mb-md-0">
                                            <span>Corregir Secciones</span><em class="icon ni ni-user-list-fill"></em>
                                        </div>
                                    @endif
...
@section('script')
    ...
    <script>
        ...
        function rowCallBack(nRow, aData, iDisplayIndex, iDisplayIndexFull) {
            ...
            @else
                $('td:eq(11)', nRow).html(
                    '<a href="/admin/promovido/' + aData['id'] +
                    '" class="btn btn-round btn-icon btn-outline-light"><em class="icon ni ni-user"></em></a>'
                );
            @endif
        }
        ...
        function updateSections() {
            ...
            //updateDataTable(tablePath + params);
            $.ajax({
                url: '{{ route('promoted.updatePromotedSectionsSearch') }}' + '?type=7' + params,
                method: 'GET',
                beforeSend: function() {
                ...
                success: function(response) {
                    //console.log(JSON.parse(response)); // Mostrar el JSON en la consola
                    showSuccessModal(response.message);
                    // Puedes agregar código aquí para manipular la respuesta y llenar la tabla si es necesario
                },
